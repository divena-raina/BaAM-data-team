...
excluding Altgeld Gardens (community area 54).
"""
...
def load_dataset() -> pd.DataFrame:
    ...
    # Exclude Altgeld Gardens (community area 54)
    df = df[df["community_area"] != 54]
    
    print(
        f"Loaded dataset with {df.shape[0]:,} rows and {df.shape[1]} columns.")
    print(f"Excluded Altgeld Gardens (community area 54) from analysis.")
    return df
...
def check_community_area_coverage(df: pd.DataFrame) -> None:
    ...
    expected.discard(54)  # Remove Altgeld Gardens from expected
    observed = set(df["community_area"].unique())
    missing = sorted(expected - observed)
    if missing:
        print(f"WARNING: Missing community areas in dataset: {missing}")
    else:
        print("All community areas (1–77, excluding 54) are represented in the dataset.")
